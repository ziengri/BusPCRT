...
UNIT_NAME="buspcrt-processor.service"
...
usage() {
  cat <<'EOF'
Usage:
  processor_service.sh install --project-root <path> --env <processor.env> --python <python_bin>
  processor_service.sh uninstall
EOF
}
...
ExecStart=${python_bin} app/run_processor.py --env-file ${env_file}
Restart=always
RestartSec=1
