...
UNIT_NAME="buspcrt-monitor.service"
...
usage() {
  cat <<'EOF'
Usage:
  monitor_service.sh install --project-root <path> --env <monitor.env> --python <python_bin>
  monitor_service.sh uninstall
EOF
}

install_service() {
  local project_root="" env_file="" python_bin=""
  ...
  cat >"$UNIT_PATH" <<EOF
[Unit]
Description=BusPCRT Monitor Service
...
ExecStart=${python_bin} app/run_monitor.py --env-file ${env_file}
Restart=always
RestartSec=5
