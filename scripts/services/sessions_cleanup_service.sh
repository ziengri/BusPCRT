...
UNIT_NAME="buspcrt-sessions-cleanup.service"
...
usage() {
  cat <<'EOF'
Usage:
  sessions_cleanup_service.sh install --project-root <path> --env <config.env>
  sessions_cleanup_service.sh uninstall
EOF
}
...
install_service() {
  ...
  [[ -f "$project_root/scripts/cleanup_sessions.sh" ]] || { log_error "Cleanup script not found: $project_root/scripts/cleanup_sessions.sh"; exit 1; }

  log_info "Writing unit: $UNIT_PATH"
  cat >"$UNIT_PATH" <<EOF
[Unit]
Description=BusPCRT Sessions Cleanup Service
...
ExecStart=/bin/bash ${project_root}/scripts/cleanup_sessions.sh --config-env-file ${env_file}
